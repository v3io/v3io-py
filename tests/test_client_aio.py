...
class Test(unittest.IsolatedAsyncioTestCase):
    ...
    async def _delete_dir(self, path):
        response = await self._client.container.list(
            container=self._container, path=path, raise_for_status=v3io.aio.dataplane.RaiseForStatus.never
        )
        ...
        for common_prefixes in response.output.common_prefixes:
            await self._client.object.delete(container=self._container, path=common_prefixes.prefix)
...
class TestContainer(Test):
    ...
    async def test_get_container_contents_invalid_path(self):
        response = await self._client.container.list(
            container=self._container, path="/no-such-path", raise_for_status=v3io.aio.dataplane.RaiseForStatus.never
        )
    ...
    async def test_get_container_contents(self):
        ...
        response = await self._client.container.list(
            container=self._container, path=self._path, get_all_attributes=True
        )
...
class TestStream(Test):
    async def asyncSetUp(self):
        ...
        await self._client.stream.delete(
            container=self._container, stream_path=self._path, raise_for_status=[200, 204, 404]
        )
    ...
    async def test_delete_stream_with_cg(self):
        ...
        # create a stream
        await self._client.stream.create(container=self._container, stream_path=self._path, shard_count=num_shards)

        # write data to all shards so there are files
        for shard_id in range(num_shards):
            await self._client.stream.put_records(
                container=self._container,
    ...
    async def test_stream(self):
        ...
        response = await self._client.stream.put_records(
            container=self._container, stream_path=self._path, records=records
        )
        ...
        response = await self._client.stream.seek(
        ...
        response = await self._client.stream.get_records(
        ...
        await self._client.stream.update(container=self._container, stream_path=self._path, shard_count=16)
    ...
    async def _stream_exists(self):
        response = await self._client.stream.describe(
            container=self._container, stream_path=self._path, raise_for_status=v3io.aio.dataplane.RaiseForStatus.never
        )
...
class TestObject(Test):
    ...
    async def test_object(self):
        ...
        response = await self._client.object.get(
            container=self._container, path=self._object_path, raise_for_status=v3io.aio.dataplane.RaiseForStatus.never
        )

        self.assertEqual(404, response.status_code)

        # put contents to some object
        await self._client.object.put(container=self._container, path=self._object_path, body=contents)

        # get the contents
        response = await self._client.object.get(container=self._container, path=self._object_path)
        ...
        response = await self._client.object.head(container=self._container, path=self._object_path)

        self.assertIn(("Content-Length", str(len(contents))), response.headers.items())

        # get the head of the dir-object
        response = await self._client.object.head(container=self._container, path=self._object_dir)

        self.assertIn(("Content-Length", str(0)), response.headers.items())

        # delete the object
        await self._client.object.delete(container=self._container, path=self._object_path)
    ...
    async def test_append(self):
        ...
        for content in contents:
            await self._client.object.put(container=self._container, path=self._object_path, body=content, append=True)
    ...
    async def test_get_offset(self):
        await self._client.object.put(container=self._container, path=self._object_path, body="1234567890")

        # get the contents without limit
        response = await self._client.object.get(container=self._container, path=self._object_path, offset=4)

        self.assertEqual(response.body.decode("utf-8"), "567890")

        # get the contents with limit
        response = await self._client.object.get(
            container=self._container, path=self._object_path, offset=4, num_bytes=3
        )
...
class TestSchema(Test):
    ...
    async def test_create_schema(self):
        await self._client.kv.create_schema(
            container=self._container,
        ...
        # verify the scehma
        response = await self._client.object.get(
            container=self._container, path=self._schema_path, raise_for_status=v3io.aio.dataplane.RaiseForStatus.never
        )
...
class TestKv(Test):
    ...
    async def test_kv_array(self):
        ...
        await self._client.kv.put(container=self._container, table_path=self._path, key=item_key, attributes=item)

        for attribute_name in item.keys():
            await self._client.kv.update(
                container=self._container,
        ...
        # get the item
        response = await self._client.kv.get(container=self._container, table_path=self._path, key=item_key)
    ...
    async def test_kv(self):
        ...
        response = await self._client.kv.get(
            container=self._container,
            table_path=self._path,
            key="louise",
        ...
        response = await self._client.kv.scan(
    ...
    async def test_limit(self):
        ...
        # limit using all()
        received_items = await self._client.kv.new_cursor(
            container=self._container, table_path=self._path, limit=30
        ).all()
    ...
    async def _delete_items(self, path, items):
        ...
        # delete dir
        await self._client.object.delete(container=self._container, path=path)
...
class TestRaiseForStatus(Test):
    ...
    async def test_always_raise_no_error(self):
        ...
        await self._client.container.list(
    ...
    async def test_never_raise(self):
        await self._client.object.get(
            container=self._container, path="/non-existing", raise_for_status=v3io.aio.dataplane.RaiseForStatus.never
        )
